...
describe("Unit test update customer use case", () => {
    it("should update a customer", async () => {
        ...
        const findCustomerSpy = jest.spyOn(customerRepository, 'find')
    ...
    it("should thrown an error when name is missing", async () => {
        ...
        await expect(upateCustomerUseCase.execute(input)).rejects.toThrow("Name is required");
        expect(findCustomerSpy).toHaveBeenCalledWith(customer.id)
    });
    ...
    it("should thrown an error when address is missing",async () => {
        ...
        await expect(upateCustomerUseCase.execute(input)).rejects.toThrow("Street is required");
        expect(findCustomerSpy).toHaveBeenCalledWith(customer.id) 
    });
});
